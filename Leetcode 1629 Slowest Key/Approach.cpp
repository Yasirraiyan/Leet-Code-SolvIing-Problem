        ...
        */
        int maxtime=findmax(reserve);
          ...
          for(int m=0;m<reserve.size();m++)
          {
            ...
            {
              //ans=m;     
               maxKey = max(maxKey, keysPressed[m]);       // Take lexicographically larger key if there are ties jemon a,c er moddhe pore ba larger lexicographically c 
                                                         tai eta c return kore
              }
          }
       ...
       /* for(int k=0;k<keysPressed.size();k++)
        {
          if(k==ans)
          {
             resultt = max(resultt, keysPressed[k]); // Take lexicographically larger key if there are ties jemon a,c er moddhe pore ba larger lexicographically c 
                                                         tai eta c return kore
